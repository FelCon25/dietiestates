...
class AddAssistantScreenViewModel(
    ...
    fun addAssistant(){
        if(_state.value.email.isEmpty()){
            onEvent(AddAssistantScreenEvent.OnWrongValueTextField("Please insert a valid email"))
        }
        else if(_state.value.firstName.isEmpty()){
            onEvent(AddAssistantScreenEvent.OnWrongValueTextField("Please insert a valid first name"))
        }
        else if(_state.value.lastName.isEmpty()){
            onEvent(AddAssistantScreenEvent.OnWrongValueTextField("Please insert a valid last name"))
        }
        else if(_state.value.password.length < 8){
            onEvent(AddAssistantScreenEvent.OnWrongValueTextField("Please enter a password of at least 8 characters!"))
        }
        else{
            viewModelScope.launch {
                        ...
                        onError { error ->
                            when(error){
                                is DataError.Remote.CustomError -> onEvent(
                                    AddAssistantScreenEvent.OnAddAssistantFailed(error.errorMessage))
                                else -> onEvent(AddAssistantScreenEvent.OnAddAssistantFailed("There was an error while creating the new assistant, please try again."))
                            }
                        }
